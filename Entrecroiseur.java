...
public class Entrecroiseur<T extends Individu<?>> {
    private BiFunction<T, T, List<T>> fabriqueDescsendants;

    public Entrecroiseur(BiFunction<T, T, List<T>> fonctionFabriqueDeDescendants) {
        this.fabriqueDescsendants = fonctionFabriqueDeDescendants;
    }
    ...
    public Generation<T> getGeneration(Generation<T> generationParent) {
        ...
        while(nouvelleGeneration.nbIndividus() < 50 && index < generationParent.nbIndividus() -1 ){
            ...
            nouvelleGeneration.addGroupIndividus(fabriqueDescsendants.apply(generationParent.getIndividus().get(index),generationParent.getIndividus().get(index+1)));     
            index +=2;        
            }
